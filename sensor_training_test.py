...
### Some examples of possible tests to train the Sensor policy:

### Test the environment
from utils import get_screen, ReplayMemory
from utils import Sensor_Q_Net, choose_action_epsilon_greedy

### Initialize the policy network
num_feat = 8
action_space_dim = 6
sensor_policy_net = Sensor_Q_Net(action_space_dim)



#Training for LEVEL A:
from utils import train_sensor_policy_PSNR
beta = 0.5
num_epochs = 5000

sensor_policy_net = Sensor_Q_Net(action_space_dim)
sensor_policy_net, average_reward, average_length = train_sensor_policy_PSNR(sensor_policy_net, beta, num_epochs, model_mq, h,w)



#Training for LEVEL B:
from utils import PhysicalValueRegressor
regressors = ['models/state_estimator_64.pt',
             'models/state_estimator_32.pt',
             'models/state_estimator_16.pt',
             'models/state_estimator_8.pt',
             'models/state_estimator_4.pt',
             'models/state_estimator_2.pt']
regressor_mq = []
for i in range(6):
    reg = PhysicalValueRegressor(512, 4)
    reg.load_state_dict(torch.load(regressors[i]))
    regressor_mq.append(reg)

from utils import train_sensor_policy_semantically
beta = 0.5
num_epochs = 5000
sensor_policy_net = Sensor_Q_Net(action_space_dim)
sensor_policy_net, average_reward, average_length = train_sensor_policy_semantically(sensor_policy_net, beta, num_epochs, model_mq, regressor_mq,h,w)